...
class HomeScreen(private val ui: TvUi, private val nav: MiruroNavigator) {
    ...
    private fun hero(root: LinearLayout, item: AnimeItem) = FrameLayout(root.context).apply {
        ...
        addView(LinearLayout(context).apply {
            ...
            addView(ui.body("Browse trending anime, seasonal releases, top rated series, and movies with a TV-first interface."))
...
class SettingsScreen(private val ui: TvUi) { fun render(root: LinearLayout) { root.addView(ui.state("Miruro Anime is a standalone AniList browser. Playback only starts when an episode already has a direct playable URL. This build does not include protected extraction, Cloudstream dependencies, or loadLinks copies."), LinearLayout.LayoutParams(-1, root.context.dp(210)).apply { topMargin = root.context.dp(24) }) } }
